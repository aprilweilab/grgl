...
public:
    ...
    explicit GRG(const size_t numSamples, const uint16_t ploidy, const bool phased = true)
...
public:
     ...
     */
    explicit MutableGRG(const size_t numSamples,
                        const uint16_t ploidy,
                        size_t initialNodeCapacity = DEFAULT_NODE_CAPACITY,
                        const bool phased = true)
...
public:
    explicit CSRGRG(const size_t numSamples,
                    const size_t nodeCount,
                    const uint16_t ploidy,
                    const bool loadUpEdges = true,
                    const bool phased = true)
        : GRG(numSamples, ploidy, phased),
